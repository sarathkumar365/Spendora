...
mod tests {
    ...
    async fn transactions_handler_applies_filters() {
        ...
        sqlx::query("INSERT INTO transactions (id, account_id, external_txn_id, amount_cents, currency_code, description, booked_at, amount, details, transaction_date, source, classification_source, explanation) VALUES ('tx-api-1', ?1, 'h-api-1', 1250, 'CAD', 'Coffee Run', '2026-03-10', '12.50', 'Coffee Run', '2026-03-10', 'manual', 'manual', 'manual import')")
            .bind(&account_id)
            .execute(&pool)
            .await
            .expect("insert tx1");
        sqlx::query("INSERT INTO transactions (id, account_id, external_txn_id, amount_cents, currency_code, description, booked_at, amount, details, transaction_date, source, classification_source, explanation) VALUES ('tx-api-2', ?1, 'h-api-2', 8200, 'CAD', 'Salary', '2026-03-11', '82.00', 'Salary', '2026-03-11', 'manual', 'manual', 'manual import')")
            .bind(&account_id)
            .execute(&pool)
            .await
            .expect("insert tx2");
